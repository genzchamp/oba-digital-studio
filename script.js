...
(function(){
  ...
  addMeta('property=og:site_name','OBA Digital Studio');
  ...
  if(!document.querySelector('script[type="application/ld+json"]')){
    ...
    ld.textContent=JSON.stringify({
      ...
      '@type':'Organization',
      'name':'OBA Digital Studio',
      'url':window.location.origin+'/oba-digital-studio/',
      'description':description,
      'contactPoint':{'@type':'ContactPoint','telephone':'+2348123958585','contactType':'customer service'}
    });
...
function handleProduct(button){
  ...
  const message=PRODUCT_MESSAGES[key]||"Hi OBA Digital Studio, I'm interested in buying one of your digital products. Please send me the price and payment instructions.";
  window.open(wa(message),'_blank','noopener')
}
document.querySelectorAll('.buy-large,[data-selar]').forEach(button=>{
...
/* WhatsApp CTAs: data-service is authoritative. Never let an old contact.html/# href override it. */
...
if(mainWA&&!mainWA.dataset.noWhatsapp){
  mainWA.href=mainWA.dataset.service?wa(SERVICE_MESSAGES[mainWA.dataset.service]):wa(SERVICE_MESSAGES.general);
  mainWA.target='_blank';
  mainWA.rel='noopener';
}
